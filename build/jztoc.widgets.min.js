...
this._updateData()},_createTableHeader:function(a){tr=$("<tr></tr>");for(var b in a)this._createHeaderCell(tr,b,a[b]).appendTo(tr);tr.appendTo(this.tableHead)},_createHeaderCell:function(a,b,c){var e=$("<th></th>");e.addClass("col"+b);e.addClass("header");e.attr("data-field",c.field);if(c.customHeaderAttributes!=null)try{e.attr(c.customHeaderAttributes)}catch(d){console.log(d)}var f=null;if(c.headerRenderer!=null)f=c.headerRenderer;else if(this.options.headerRenderer!=null)f=this.options.headerRenderer;
if(f!=null){a={listData:{owner:this,data:null,column:c,index:b,th:e,tr:a}};c.itemRendererOptions!=null&&jQuery.extend(a,c.headerOptions);e[f](a)}else e.html(c.label);c.headerAttr!=null&&e.attr(c.headerAttr);c.headerStyle!=null&&this._applyStyle(e,c.headerStyle);c.width!=null&&e.css("width",c.width);c.visible==false&&e.hide();return e},_updateData:function(){$(this.tableBody).children().remove();var a=this.options.dataProvider,b;for(b in a){var c=a[b],e=this._buildRow(b,c,this.tableBody),d;for(d in this.options.columns){var f=
this._buildCell(e,b,b,c,this.options.columns[d]);e.append(f)}}},_buildRow:function(a,b,c){c=$("<tr></tr>").appendTo(c);var e=this._guid();c.attr("data-uid",e);this.options.rowFunction!=null&&this.options.rowFunction(c,a,b);return c},_buildCell:function(a,b,c,e,d){var f=$("<td></td>");f.addClass("col"+c);f.addClass("column");f.attr("data-field",d.field);if(d.width!=null)try{f.css("width",d.width)}catch(g){console.log(g)}this._applyStyle(f,d.style);if(d.customAttributes!=null)try{f.attr(d.customAttributes)}catch(h){console.log(h)}c=
